...
const s3 = `37	1069	166	1037	172	258	1071	90	497	1219	145	1206	143	153	1067	510`;
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    for (var j = 0; j < row.length; j++) {
      for (var k = j + 1; k < row.length; k++) {
        if (row[k] % row[j] == 0) {
          div.push(row[k] / row[j]);
          k = j = row.length;
        } else if (row[j] % row[k] == 0) {
          div.push(row[j] / row[k]);
          k = j = row.length;
        }
      }
    }
  }
  return div.reduce((acc, curr) => acc + curr);
}
